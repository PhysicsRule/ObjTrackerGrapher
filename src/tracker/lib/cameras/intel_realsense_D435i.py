...
class IntelRealSenseD435i(Camera):
    ...
    def select_clipping_distance(self, frame, rs_depth) -> Tuple[Any, float, float]:
    ...
    def get_coordinates_meters(self, rs_frame, rs_depth, x_pixel: int, y_pixel: int, depth) -> Tuple[float, float, float]:
    ...
    def select_location(self, frame, rs_frame, rs_depth) -> Tuple[float, Tuple[float, float, float], float]:
    # This selects both the background and the TODO  Origin
         # Find distance (depth) to wall & store as clipping distance
        # Note that it is reduced slightly to allow some room for error (*0.95)
        wall, depth, clipping_distance = self.select_clipping_distance(frame, rs_depth)
        depth = round(rs_depth.get_distance(int(wall[0]),int(wall[1])),4)
        frame_point = self.get_coordinates_meters(rs_frame, rs_depth, wall[0], wall[1], depth)
        return clipping_distance, frame_point, depth

    def get_depth_meters(self, x_pixel, y_pixel, radius_meters, rs_depth, rs_frame, zeroed_x, zeroed_y, zeroed_z, clipping_distance) -> Tuple[Optional[float], Optional[float], Optional[float]]:
        try:
        ...
        frame_point = self.get_coordinates_meters(rs_frame, rs_depth, x_pixel,  y_pixel, depth)
    ...
    def select_furthest_distance_color(self, pipeline) -> Tuple[float, float, float, float]:
        ...
        while clipping_distance < 0.1:
            ...
            clipping_distance, color_point, zeroed_z = self.select_location(cv_color, rs_color, rs_depth)
            ...
            zeroed_x, zeroed_y, zeroed_z = self.set_the_origin (color_point)
    ...
    def select_furthest_distance_infrared(self, pipeline):
        ...
        while clipping_distance < 0.1:
            ...
            # Note that it is reduced slightly to allow some room for error (*0.95)
             # Find the clipping distance
            clipping_distance, depth_point, zeroed_z = self.select_location(infrared_image, rs_infrared, rs_depth)
            #TODO later call above to find the origin then call the following after
            # for now this just sets all zeroed values as 0 instead of a specific origin, so the origin is the cent of the camera
            zeroed_x, zeroed_y, zeroed_z = self.set_the_origin (depth_point)
